"""
Exact same code as zero_depot_deadheading.py, except the objective function has
changed only to remove the deadhead weighting. Offers an answer to the fleet minimization problem,
INSENSITIVE to operational costs due to deadheading.
"""
...
prob.setObjective(pulp.lpSum( var[str(arc)]
                             for _, arc in weights_and_arcs))
...
# It follows that the minimum fleet size in zero_depot_deadheading was NOT an artifact
# of the weighting chosen!
...
arc_departures = {str(a): str(b) for (_, (a,b)) in weights_and_arcs 
...
# deadheading time in comparison to the WEIGHTED case?
deadhead_duration = 0
...
# Absurdly long deadheading time, at over 61 runtime hours per day!
assert(deadhead_duration == '61:16:41')
